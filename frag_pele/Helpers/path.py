...
class PathHandler:
    def __init__(self,
        ...
        self._working_dir = os.path.join(current_path, "{}_{}".format(pdb_basename, ID))

        if not os.path.exists(working_dir):
            os.mkdir(working_dir)  # Creating a working directory for each PDB-fragment combination

        self._pdbout_folder = os.path.join(working_dir, pdbout)
        self._path_to_templates_generated, self._path_to_templates = self._get_path_to_templates_generated(working_dir, force_field)
        self._path_to_lib = os.path.join(working_dir, "DataLocal/LigandRotamerLibs")
        self._create_output_folder(self._working_dir)
        self._create_symbolic_links(data,documents,working_dir)
    ...
    def _get_path_to_templates_generated(self, working_dir, force_field):
        from frag_pele.Erros.custom_errors import WrongForceField
